...
export default function Projects({ projects, locale }: ProjectsProps) {
  ...
  return (
    <section id="projects" ref={ref} className="px-6 py-16 md:px-8 md:py-24 xl:px-16 xl:py-32">
      <motion.div
      ...
      >
        <h2 className="font-black uppercase tracking-tight text-4xl sm:text-5xl md:text-6xl xl:text-[7rem] text-zinc-900">
          {projects.title_line1}
        </h2>
        <h2 className="font-black uppercase tracking-tight text-4xl sm:text-5xl md:text-6xl xl:text-[7rem] text-zinc-200">
          {projects.title_line2}
        </h2>
      </motion.div>

      <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
        {projects.projects.map((project, i) => (
          <Link key={project.slug} href={`/${locale}/projects/${project.slug}`} className="group rounded-2xl overflow-hidden">
          <motion.div
